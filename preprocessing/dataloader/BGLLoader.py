...
class BGLLoader(BasicDataLoader):
    ...
    def _load_raw_log_seqs(self):
        ...
        anomalous_file = os.path.join(self.dataset_base, 'block2anomolous.txt')
        if os.path.exists(sequence_file) and os.path.exists(label_file) and os.path.exists(anomalous_file):
            ...
            with open(anomalous_file, 'r', encoding='utf-8') as reader:
                for line in reader.readlines():
                    block_id, seq = line.strip().split(':')
                    if block_id not in self.block2anomalous.keys():
                        self.block2anomalous[block_id] = []
                    self.block2anomalous[block_id] = [int(x) for x in seq.split()]
        else:
            ...
            with open(self.in_file, 'r', encoding='utf-8') as reader:
                ...
                for node, seq in nodes.items():
                    if len(seq) < self.window_size:
                        ...
                        self.block2anomalous[str(block_idx)] = []
                        label = 'Normal'
                        for (idx, line) in seq:
                            self.block2seqs[str(block_idx)].append(int(idx))
                            if not line.startswith('-'):
                                label = 'Anomalous'
                                self.block2anomalous[str(block_idx)].append(1)
                            else:
                                self.block2anomalous[str(block_idx)].append(0)
            ...
            with open(anomalous_file, 'w', encoding='utf-8') as writer:
                for block in self.block2anomalous.keys():
                    writer.write(':'.join([block, ' '.join([str(x) for x in self.block2anomalous[block]])]) + '\n')
